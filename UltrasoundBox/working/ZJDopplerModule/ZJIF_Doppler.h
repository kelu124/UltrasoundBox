// == [6/25/2015 AlexHuang]
// 增加SetPara2和GetPara2接口，因为不定参传递过于麻烦
#pragma once
...
public:
	...
	virtual int DInit() = 0;	//初始化D模式
	virtual int DLoadParas(DParas *pparas) = 0;	//加载D模式参数
	virtual int DLoadParasFromFile(char *file) = 0;	//从文件加载D模式参数
	virtual int DSaveParasToFile(char *file) = 0;	//将D模式参数保存到文件中
